...
KeyboardConfigParser::KeyboardConfigParser() {
	this->bindings = {
...
KeyboardConfig* KeyboardConfigParser::parse(std::stringstream& configContents) {
	...
	return new KeyboardConfig(this->right(), this->left(), this->jump(), this->restart());
}

void KeyboardConfigParser::mapToKey(std::string& key, std::string& value) {
	if (this->isBindingSupported(key) && this->isKeySupported(value))
		this->setBinding(key, value);
}

const std::map<std::string, sf::Keyboard::Key> KeyboardConfigParser::keyMap = {
	{"A", sf::Keyboard::Key::A},
	{"B", sf::Keyboard::Key::B},
	{"C", sf::Keyboard::Key::C},
	{"D", sf::Keyboard::Key::D},
...
bool KeyboardConfigParser::isBindingSupported(std::string& key) {
	return this->bindings.count(key);
}

bool KeyboardConfigParser::isKeySupported(std::string& key) {
	return this->keyMap.count(key);
}

void KeyboardConfigParser::setBinding(std::string& binding, std::string& key) {
	this->bindings[binding] = this->keyMap.at(key);
}

sf::Keyboard::Key KeyboardConfigParser::getBindingKey(const std::string& binding) {
	return this->bindings[binding];
}

sf::Keyboard::Key KeyboardConfigParser::left() {
	return this->getBindingKey(LEFT_KEY);
}

sf::Keyboard::Key KeyboardConfigParser::right() {
	return this->getBindingKey(RIGHT_KEY);
}

sf::Keyboard::Key KeyboardConfigParser::jump() {
	return this->getBindingKey(JUMP_KEY);
}

sf::Keyboard::Key KeyboardConfigParser::restart() {
